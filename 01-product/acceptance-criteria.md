...
| AC-53 | Given a scripted set of positions where Level 1 or Level 2 is hopeless (only king and pawns left, 9 or more points behind) and has a legal king move, then in at least half of its moves it moves its king to a square closer to the nearest child piece | v0.1 |
| AC-54 | Given Level 1 or Level 2 has been hopeless (only king and pawns left, 9 or more points behind) after each of its last 3 moves, then it resigns; with any knight, bishop, rook or queen left it never resigns. When hopeless, it resigns before its next move: the game ends as a win for the child, the creature plays a big Sad animation (about 3 seconds), and the result counts as a win everywhere a win counts. Levels 3 to 8 never resign | v0.1 |
