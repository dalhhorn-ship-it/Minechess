...
| TD-04 | **Done:** the AI worker now falls back to a random legal move on error or after 6 seconds, and is always stopped when a game closes | M | Done | Add `worker.onerror` and a timeout that falls back to a random legal move |
